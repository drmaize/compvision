function [bw_conn, connections] = Connect_MST_ND(bw, gap_length, scale, endpointsOnly, window_sz, verbose)
...
if(all(window_sz <= 1))
    ...
    [bw_conn, connections] = Connect_MST_ND_Helper(bw,gap_length, scale, endpointsOnly);
else
    ...
    for ii=1:numCells
        ...
        if length(find(outCell{ii})) > 5
            [outCell{ii}, connections] = Connect_MST_ND_Helper(outCell{ii},gap_length, scale, endpointsOnly);
        end
    end
    if(verbose)
...
function [bw_conn connections] = Connect_MST_ND_Helper(bw,gap_length, scale, endpointsOnly)
