...
public class Fusion 
{
	...
	public static < T extends RealType< T > > ImagePlus fuse( final T targetType, final ArrayList< ImagePlus > images, final ArrayList< InvertibleBoundable > models, 
			final int dimensionality, final boolean subpixelResolution, final int fusionType, final String outputDirectory, final boolean noOverlap, int gridSizeX, int gridSizeY )
	{
		...
		final int numTimePoints = images.get( ind ).getNFrames();
		final int numChannels = images.get( ind ).getNChannels();
			...
			for ( int c = 1; c <= numChannels; ++c )
			{
				...
				if ( subpixelResolution )
				{
					...
					else
					{
						...
						writeBlock( out, numSlices, t, numTimePoints, c, numChannels, blockData, offset, models, fusion, outputDirectory,gridSizeX,gridSizeY );
						out.close();
					}
				}
				else
...
A:        					for ( int i = 0; i < numImages; ++i )
	...
	protected static <T extends RealType<T>> void writeBlock( final Image<T> outputSlice, final int numSlices, final int t, final int numTimePoints, final int c, final int numChannels, 
			...
			final ArrayList< InvertibleBoundable > transform, final PixelFusion fusion, final String outputDirectory, int gridSizeX, int gridSizeY )
	{
		final int numImages = input.size();
		...
		IJ.log("offset: " + offset[0] + ", " + offset[1] + ", " + offset[2]);
		
		float gx = outputSlice.getDimension(0)/(float)gridSizeX;//10.0;
		
		float gy = outputSlice.getDimension(1)/(float)gridSizeY;//10.0;
		...
		/*for ( int i = 0; i < numImages; ++i )
		{
			...
			{
				IJ.log("gx:" + gx + ", gy:" + gy);
				gx = gx < input.get(i).getImage().getDimension(0) ? input.get(i).getImage().getDimension(0) : gx;
				gy = gy < input.get(i).getImage().getDimension(1) ? input.get(i).getImage().getDimension(1) : gy;
				IJ.log("gx:" + gx + ", gy:" + gy);
				break;
			...
			for ( int slice = 0; slice < numSlices; ++slice )
			{
				...
				while ( out.hasNext() )
				{
  				...
  				//updateStatus(globalProgress, localProgress, stepsTaken, imageSize, outputSlice);
					...
					// loop over all images for this output location
					int px = (int)((tmp[0]-offset[0])/gx);
					int py = (int)((tmp[1]-offset[1])/gy);
					
					int loc = py*gridSizeX + px;
					int[] locs = {loc, loc-1,loc+1,loc-gridSizeX,loc+gridSizeX,loc-(gridSizeX+1),loc+(gridSizeX+1),loc-(gridSizeX-1),loc+(gridSizeX-1)};
A:		        	for ( int i = 0; i < locs.length; ++i )
		        	{
		        		if (locs[i]<0 || locs[i] >= numImages || in.get(locs[i]) == null )
		        			continue;
						float[] tmp1 = transform.get( locs[i] ).applyInverse( tmp );
		            	
		        		// test if inside
						for ( int d = 0; d < numDimensions; ++d )
							if ( tmp1[ d ] < 0 || tmp1[ d ] > max[ locs[i] ][ d ] )
								continue A;
						//IJ.log("image: " + locs[i] + "; grid: " + px + ", " + py);
						in.get( locs[i] ).setPosition( tmp1 );			
						myFusion.addValue( in.get( locs[i] ).getType().getRealFloat(), locs[i], tmp1 );
					}
					
					// set value
					out.getType().setReal( myFusion.getValue() );
				}
				IJ.log("Time for fusing slice : " + (System.currentTimeMillis() - time));
				// write the slice
				...
				IJ.log("Time for writing slice : " + (System.currentTimeMillis() - time));
			}
		} 
		catch ( NoninvertibleModelException e ) 
		{
