...
public class PairWiseStitchingImgLib 
{
	...
	public static < T extends RealType<T>, S extends RealType<S> > PairWiseStitchingResult performStitching( final Image<T> img1, final Image<S> img2, final StitchingParameters params )
	{
		...
		final PairWiseStitchingResult result = computePhaseCorrelation( img1, img2, params.checkPeaks, params.subpixelAccuracy, params.ignoreZStage );
	...
	public static < T extends RealType<T>, S extends RealType<S> > PairWiseStitchingResult computePhaseCorrelation( final Image<T> img1, final Image<S> img2, final int numPeaks, final boolean subpixelAccuracy, final boolean ignoreZ )
	{
		IJ.log("Inside modified pairwise matching function");
		IJ.log("ignoreZ : " + ignoreZ);
		IJ.log("subpixel accuracy : " + subpixelAccuracy);
		final PhaseCorrelation< T, S > phaseCorr = new PhaseCorrelation<T, S>( img1, img2, ignoreZ );
		phaseCorr.setInvestigateNumPeaks( numPeaks );
