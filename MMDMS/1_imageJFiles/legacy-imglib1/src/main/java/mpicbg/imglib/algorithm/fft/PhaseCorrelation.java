...
public class PhaseCorrelation<T extends RealType<T>, S extends RealType<S>> implements MultiThreaded, Algorithm, Benchmark
{
	...
	public PhaseCorrelation( final Image<T> image1, final Image<S> image2, final boolean ignoreZ )
	{
		this( image1, image2, 5, true );
		this.ignoreZ = ignoreZ;
		//IJ.log("inside new constructor");
	}
	...
	protected ArrayList<PhaseCorrelationPeak> extractPhaseCorrelationPeaks( final Image<FloatType> invPCM, final int numPeaks,
		...
		IJ.log("In edited phase corelation peak search... ignorez:" + ignoreZ + "offset" + offset);
		cursor. setPosition(offset);
		while ( (!ignoreZ || cursor.getPosition()[2] == offset[2]) && cursor.hasNext())
		{
			cursor.fwd();
